...
class C2VCellClassifier(CellClassifier):
    def __init__(self, config):
        ...
        cl_model_path = config["c2v"]['cl_model']
    ...
    def get_result(self, table, senc, tarr):
        labels, probs, features = predict_labels(table, self.cl_model, self.ce_model, self.fe_model, senc, self.mode, self.device)
        probs = np.exp(probs)
        labels = np.vectorize(lambda x: self.label2ind[x])(labels)
        result = dict(text=tarr.tolist(), labels=labels.tolist(), labels_probs=probs.tolist())

        return result, features

    def generate_c2v_from_array(self, tarr, tfeat):
        senc = SentEnc(self.infersent_model, self.w2v_path, self.vocab_size, device=self.device, hp=False)
        prep = Preprocess()
